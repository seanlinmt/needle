...
def _install_xla_log_filter():
    ...
    sys.stderr = os.fdopen(
        py_stderr_fd, "w", encoding="utf-8", errors="replace", buffering=1
    )
...
def main():
    ...
    p.add_argument(
        "--query", type=str, default=None, help="Query text for tool-call generation"
    )
    p.add_argument(
        "--tools", type=str, default=None, help="Tools JSON for tool-call generation"
    )
    ...
    p.add_argument(
        "--num-samples", type=int, default=500, help="Number of samples to generate"
    )
    p.add_argument(
        "--batch-size", type=int, default=25, help="Examples per Gemini call"
    )
    ...
    p.add_argument(
        "--dry-run", action="store_true", help="Generate only, skip save and upload"
    )
    p.add_argument(
        "--output-jsonl",
        type=str,
        default=None,
        help="Also save raw generations to JSONL",
    )
    p.add_argument(
        "--upload-every", type=int, default=None, help="Merge+upload every N samples"
    )
    ...
    p = sub.add_parser("api", add_help=False)
    p.add_argument("--checkpoint", type=str, default=None)
    ...
    tp.add_argument("--zone", type=str, default=None)
    tp.add_argument(
        "train_args", nargs=argparse.REMAINDER, help="Extra args passed to needle train"
    )
